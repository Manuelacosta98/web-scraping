...
#get the search word
string_query = sys.argv[1].replace(" ","_")
...
#indicate number of words to get
if(len(sys.argv) > 3):
	size = int(sys.argv[3])
else:
	size = 20
...
try:
    ...
    if len(data['query']['search'])>0:
    	...
    	print "Articles not found, try another string"
    	exit()
    ...
    #just get the top words_count words
    if len(sorted_word_frequency_list) > size:
        sorted_word_frequency_list = [ sorted_word_frequency_list[i] for i in range(0, size) ]
